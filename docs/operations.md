...
`meridian-lang-macro-dock` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for macro workflows, centered on state machine modeling, transition tables, and invalid-transition tests.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 5.
